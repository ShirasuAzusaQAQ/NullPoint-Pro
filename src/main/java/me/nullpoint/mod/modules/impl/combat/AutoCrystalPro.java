...
public class AutoCrystalPro extends Module {
    public static AutoCrystalPro INSTANCE;
    ...
    private final BooleanSetting antiAC =
            add(new BooleanSetting("AntiAC", false, v -> page.getValue() == Page.Calc));
    //Misc
    ...
    private final BooleanSetting allEntity =
            add(new BooleanSetting("AllEntity", false, v -> page.getValue() == Page.Misc));
    ...
    public AutoCrystalPro() {
        super("AutoCrystalPro", "Recode", Category.Combat);
        INSTANCE = this;
    ...
    public void onRender3D(MatrixStack matrixStack, float partialTicks) {
        ...
        if (AutoCrystalPro.INSTANCE.displayTarget != null && !AutoCrystalPro.INSTANCE.noPosTimer.passedMs(500)) {
            Aura.doRender(matrixStack, partialTicks, AutoCrystalPro.INSTANCE.displayTarget, color.getValue(), mode.getValue());
        }
    }
        ...
        if (list.isEmpty()) {
            ...
            for (BlockPos pos : BlockUtil.getSphere((float) range.getValue() + 1)) {
                ...
                if (!canPlaceCrystal(pos, true, false))
                    continue;
                if (antiAC.getValue() && AutoCrystal.INSTANCE.isOn() && AutoCrystal.crystalPos != null && AutoCrystal.crystalPos.equals(pos))
                    continue;
