...
public class AnchorAuraPro extends Module  {
    ...
    private final SliderSetting tickDelay =
            add(new SliderSetting("TickDelay", 100, 165, 500, 1, v -> page.getValue() == Page.General).setSuffix("ms"));
    private final EnumSetting<PlaceMode> placeMode =
            add(new EnumSetting<>("PlaceMode", PlaceMode.tick, v -> page.getValue() == Page.General));
    ...
    private final BooleanSetting bold = add(new BooleanSetting("Bold", false, v -> page.getValue() == Page.Render && render.getValue() && box.booleanValue)).setParent();
    private final SliderSetting lineWidth = add(new SliderSetting("LineWidth", 4,1,5, v-> page.getValue() == Page.Render && render.getValue() && bold.isOpen() && box.booleanValue));
    ...
    public static AnchorAuraPro INSTANCE;
    ...
    double fade = 0;
    public AnchorAuraPro() {
        super("AnchorAuraPro", Category.Combat);
        INSTANCE = this;
    ...
    public void onRender3D(MatrixStack matrixStack, float partialTicks) {
        ...
        if (INSTANCE.displayTarget != null && INSTANCE.currentPos != null) {
            Aura.doRender(matrixStack, partialTicks, INSTANCE.displayTarget, color.getValue(), mode.getValue());
        }
    }
    @Override
    ...
    public void anchor() {
        if (!timer.passedMs(tickDelay.getValueInt()))return;
        //Misc
        ...
        for (PlayerAndPredict pap : list) {
            ...
            if (canPlace(pos, range.getValue(), breakCrystal.getValue()) || getBlock(pos) == Blocks.RESPAWN_ANCHOR && getClickSideStrict(pos) != null) {
                double selfDamage;
                ...
                for (BlockPos pos : getSphere(range.getValueFloat())) {
                    for (PlayerAndPredict pap : list) {
                        ...
                        if (getBlock(pos) != Blocks.RESPAWN_ANCHOR) {
                            ...
                            boolean skip = getClickSideStrict(pos) == null;
                            CombatUtil.modifyPos = null;
                            if (skip) continue;
    ...
    public void TickBreak(BlockPos pos) {
        int anchor = InventoryUtil.findItem(Items.ENCHANTED_GOLDEN_APPLE);
        int old = 0;
        if (mc.player != null) {
            old = mc.player.getInventory().selectedSlot;
        }
        doSwap(anchor);
        clickBlocks(pos, getClickSide(pos), rotate.getValue());
        if (inventory.getValue()) {
            doSwap(anchor);
    ...
    public void globalPlace(BlockPos pos) {
        int anchor = findBlock(Blocks.RESPAWN_ANCHOR);
        int old = 0;
        if (mc.player != null) {
            old = mc.player.getInventory().selectedSlot;
    ...
    public void GlowPlaceable(BlockPos pos) {
        int glowStone = findBlock(Blocks.GLOWSTONE);
        int old = 0;
        if (mc.player != null) {
            old = mc.player.getInventory().selectedSlot;
        ...
        doSwap(glowStone);
        clickBlock(pos, getClickSide(pos), rotate.getValue());
        if (inventory.getValue()) {
            doSwap(glowStone);
            EntityUtil.syncInventory();
        } else {
    ...
    public double getAnchorDamage(BlockPos anchorPos, PlayerEntity target, PlayerEntity predict) {
        ...
        switch (calcMode.getValue()) {
            ...
            case Mio -> damage = MioExplosionUtil.anchorDamage(target, anchorPos, predict);
        }
    ...
    public void clickBlocks(BlockPos pos, Direction side, boolean rotate) {
        Vec3d directionVec = new Vec3d(pos.getX() + 0.5 + side.getVector().getX() * 0.5, pos.getY() + 0.5 + side.getVector().getY() * 0.5, pos.getZ() + 0.5 + side.getVector().getZ() * 0.5);
        if (rotate) {
