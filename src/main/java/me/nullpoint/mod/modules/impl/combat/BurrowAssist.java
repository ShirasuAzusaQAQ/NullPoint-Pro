...
public class BurrowAssist extends Module {
    ...
    // 新增：选择开启哪个模块
    private final EnumSetting<ModuleMode> moduleMode =
            add(new EnumSetting<>("ModuleMode", ModuleMode.All));
    ...
    public void onUpdate() {
        ...
        if (mc.player.isOnGround() &&
            ...
            // 根据ModuleMode设置决定开启哪个模块
            switch (moduleMode.getValue()) {
                case Bur:
                ...
                case BurPro:
                    if (BurrowPro.INSTANCE != null && BurrowPro.INSTANCE.isOn()) return;
                    if (BurrowPro.INSTANCE != null) {
                        BurrowPro.INSTANCE.enable();
                    } else {
                        // 如果BurrowPro不存在，则使用Burrow作为后备
                        if (Burrow.INSTANCE.isOn()) return;
                        Burrow.INSTANCE.enable();
                    }
                    break;
                ...
                case All:
                    // 尝试先开启BurrowPro，如果不存在则开启Burrow
                    if (BurrowPro.INSTANCE != null && !BurrowPro.INSTANCE.isOn()) {
                        BurrowPro.INSTANCE.enable();
                    } else if (Burrow.INSTANCE != null && !Burrow.INSTANCE.isOn()) {
                        Burrow.INSTANCE.enable();
                    }
                    break;
            }
    ...
    // 新增：模块模式枚举
    private enum ModuleMode {Bur, BurPro, All}
}