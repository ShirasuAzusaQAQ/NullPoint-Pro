...
public class WebAuraPro extends Module {
    public static WebAuraPro INSTANCE;
    public WebAuraPro() {
        super("WebAuraPro", Category.Combat);
        INSTANCE = this;
    }
    ...
    public final SliderSetting targetRange =
            add(new SliderSetting("TargetRange", 8.0, 0.0, 8.0, 0.1, v -> page.getValue() == Page.General));
    ...
    private boolean placeWeb(BlockPos pos) {
        ...
        if (checkMine.getValue() && (Nullpoint.BREAK.isMining(pos) || !noMine.getValue() && pos.equals(SpeedMine.breakPos))) return false;
        if (BlockUtil.getPlaceSide(pos, placeRange.getValue()) != null && mc.world.isAir(pos)) {
            ...
            if (noMine.getValue() && pos.equals(SpeedMine.breakPos)) {
