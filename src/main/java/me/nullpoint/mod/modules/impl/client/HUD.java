...
public class HUD extends Module {
    ...
    private final BooleanSetting comboBreaks = add(new BooleanSetting("ComboBreaks", true, v -> page.getValue() == Page.ELEMENTS));
    private final SliderSetting comboX = add(new SliderSetting("ComboX", 100, 0, 1000, v -> comboBreaks.isOpen() && page.getValue() == Page.ELEMENTS));
    private final SliderSetting comboY = add(new SliderSetting("ComboY", 100, 0, 1000, v -> comboBreaks.isOpen() && page.getValue() == Page.ELEMENTS));
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        // 添加ComboBreaks状态显示
        if (comboBreaks.getValue() && ComboBreaks.INSTANCE != null) {
            String controlText = "姿态模式";
            String attackText = "压制模式";

            // 根据模块状态设置颜色
            String controlColor = ComboBreaks.INSTANCE.isOn() ? "§c" : "§7";
            String attackColor = ComboBreaks.INSTANCE.isOn() ? "§7" : "§a";

            String displayText = controlColor + "[" + controlText + "] §f/ " + attackColor + "[" + attackText + "]";

            int x = comboX.getValueInt();
            int y = comboY.getValueInt();
            drawContext.drawTextWithShadow(mc.textRenderer, displayText, x, y, getColor(counter));
            counter++;
        }
        if (nullCheck()) return;
        counter = 20;
