...
import me.nullpoint.api.events.impl.LookAtEvent;
import me.nullpoint.api.utils.entity.InventoryUtil;
import me.nullpoint.api.utils.math.Timer;
import me.nullpoint.api.utils.world.BlockPosX;
import me.nullpoint.api.utils.world.BlockUtil;
import me.nullpoint.mod.modules.Module;
import me.nullpoint.mod.modules.impl.client.AntiCheat;
...
public class BurrowPro extends Module {
    public static BurrowPro INSTANCE;
    private final Timer timer = new Timer();
    private final Timer webTimer = new Timer();
    private final BooleanSetting disable = add(new BooleanSetting("Disable", true));
    private final SliderSetting delay = add(new SliderSetting("Delay", 500, 0, 1000));
    ...
    private final BooleanSetting netheriteBlock = add(new BooleanSetting("NetheriteBlock", false));
    ...
    private final BooleanSetting wait = add(new BooleanSetting("Wait", true, v -> !disable.getValue()));
    ...
    public BurrowPro() {
        super("BurrowPro", Category.Combat);
        INSTANCE = this;
    }

    @Override
    public void onUpdate() {
        if (EntityUtil.isInWeb( )) {
            webTimer.reset();
            return;
        }
    ...
    private boolean hasEntity(BlockPos pos) {
        for (Entity entity : BlockUtil.getEntities(new Box(pos))) {
            ...
            if (!entity.isAlive() || entity instanceof ItemEntity || entity instanceof ExperienceOrbEntity || entity instanceof ExperienceBottleEntity || entity instanceof ArrowEntity || entity instanceof EndCrystalEntity && breakCrystal.getValue() || entity instanceof ArmorStandEntity)
                continue;
            return true;
        }
        return false;
    ...
    private int getBlock() {
        if (inventory.getValue()) {
            if (netheriteBlock.getValue() && InventoryUtil.findBlockInventorySlot(Blocks.NETHERITE_BLOCK) != -1) {
                return InventoryUtil.findBlockInventorySlot(Blocks.NETHERITE_BLOCK);
            }
            ...
            if (netheriteBlock.getValue() && InventoryUtil.findBlock(Blocks.NETHERITE_BLOCK) != -1) {
